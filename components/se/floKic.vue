  ...
  <v-container fluid class="ma-0 pa-0">
    <v-row no-gutters>
      <v-col cols="12">
              ...
              <v-row no-gutters>
                <v-col cols="12" class="pa-1">
                  <p class="align-center font-weight-regular title mx-2">
                    <v-chip label>KIC: {{ cvalue }}</v-chip>
                  </p>

                  <p class="text-subtitle-1 font-weight-light ma-0 pa-0">
                    <span class="info--text">General:</span>
                    <span>{{ find_name_by_key(kic_algorithm_enum, kic_algorithm) }},</span>
                    <span v-if="kic_algorithm == '01'">{{ find_name_by_key(kic_spec_des_enum, kic_spec) }},</span>
                    <span v-if="kic_algorithm == '10'">{{ find_name_by_key(kic_spec_aes_enum, kic_spec) }}</span>
                  </p>
                </v-col>
              </v-row>
            </v-card-text>
        ...
        <flo-expand v-else title="KIC" :open="false" elevation="5">
          <template v-slot:title="{ title }">
            <span class="align-center font-weight-regular title mx-2">
              <v-chip label>{{ title }}: {{ cvalue }}</v-chip>
              <v-chip label>{{ find_name_by_key(kic_algorithm_enum, kic_algorithm) }}</v-chip>
              <v-chip label v-if="kic_algorithm == '01'">{{ find_name_by_key(kic_spec_des_enum, kic_spec) }}</v-chip>
              <v-chip label v-if="kic_algorithm == '10'">{{ find_name_by_key(kic_spec_aes_enum, kic_spec) }}</v-chip>
            </span>
          </template>
          ...
          <template v-slot:content="{}">
            <v-container fluid class="ma-0 pa-0">
              <v-row no-gutters>
                <v-col cols="12" class="pa-1">
                  <v-text-field v-model="cvalue" label="KIC" :rules="[testPattern]" @change="kic_text_changed($event)" :disabled="readonly || disabled" hint="Key & Algorithm Identifier For Ciphering" outlined dense persistent-hint></v-text-field>
                </v-col>
              </v-row>
                        ...
                        <v-card flat outlined>
                          <v-card-text class="ma-0 pa-0">
                            <v-container fluid class="ma-0 pa-0">
                              ...
                              <v-row no-gutters>
                                <v-col cols="12" class="pa-1">
                                  <v-select v-model="kic_algorithm" label="Algorithm" :items="kic_algorithm_enum" item-text="value" item-value="key" @change="kic_changed('kic_algorithm', $event)" :disabled="readonly || disabled" dense hide-details persistent-hint outlined></v-select>
                                </v-col>
                              </v-row>

                              <v-row v-if="kic_algorithm == '01'" no-gutters>
                                <v-col cols="12" class="pa-1">
                                  <v-select v-model="kic_spec" label="DES Spec" :items="kic_spec_des_enum" item-text="value" item-value="key" @change="kic_changed('kic_spec', $event)" :disabled="readonly || disabled" dense hide-details persistent-hint outlined></v-select>
                                </v-col>
                              </v-row>

                              <v-row v-if="kic_algorithm == '10'" no-gutters>
                                <v-col cols="12" class="pa-1">
                                  <v-select v-model="kic_spec" label="AES Spec" :items="kic_spec_aes_enum" item-text="value" item-value="key" @change="kic_changed('kic_spec', $event)" :disabled="readonly || disabled" dense hide-details persistent-hint outlined></v-select>
                                </v-col>
                              </v-row>

                              <v-row no-gutters>
                                <v-col cols="12" class="pa-1">
                                  <v-select v-model="kic_key_index" label="Keyset" :items="kic_key_index_enum" item-text="value" item-value="key" @change="kic_changed('kic_key_index', $event)" :disabled="keyset != 0 || readonly || disabled" dense hide-details persistent-hint outlined></v-select>
                                </v-col>
                              </v-row>
                            </v-container>
                          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    parseKic(hexValue) {
      console.log(`flo-kic - parseKic - enter`);
      // 1. verify 4 bytes that are all hex
      if (hexValue.length != 2) {
        console.log(`flo-kic - parseKic - wrong length`);
        return;
      }
      ...
      console.log(`flo-kic - parseKic - first - dec: ${first_byte} bits:${first_byte_bits}`);

      // let kic_algorithm_enum = [
      //   { key: "00", value: 'Algorithm known implicitly by both entities' },
      //   { key: "01", value: 'DES' },
      //   { key: "10", value: 'AES' },
      //   { key: "11", value: 'Proprietary Implementations' },
      ...
      this.kic_algorithm = `${this.getBitsBit(first_byte_bits, 2)}${this.getBitsBit(first_byte_bits, 1)}`;

      // let kic_spec_des_enum = [
      //   { key: "00", value: 'Reserved for DES as defined in previous releases' },
      //   { key: "01", value: 'Triple DES in outer-CBC mode using two different keys' },
      //   { key: "10", value: 'Triple DES in outer-CBC mode using three different keys' },
      //   { key: "11", value: 'Reserved for DES as defined in previous releases' },
      ...
      this.kic_spec = `${this.getBitsBit(first_byte_bits, 4)}${this.getBitsBit(first_byte_bits, 3)}`;
      ...
      // b6,b7,b8 - index
      if (this.keyset == 0) {
        this.kic_key_index = `${this.getBitsBit(first_byte_bits, 8)}${this.getBitsBit(first_byte_bits, 7)}${this.getBitsBit(first_byte_bits, 6)}${this.getBitsBit(first_byte_bits, 5)}`;
      } else {
        console.log(`flo-kic - parseKic - used keyset:${this.keyset}`);
        ...
        this.kic_key_index = l_set;
        if (l_current != l_set) {
          console.log(`flo-kic - parseKic - differ l_set:${l_set} l_current${l_current}`);
          this.kic_changed("kic_key_index", l_set);
        }
      }
    },
    kic_text_changed(event) {
      console.log(`flo-kic - kic_text_changed - enter`);
      this.parseKic(event);
    },
    kic_changed() {
      console.log(`flo-kic - kic_changed - enter`);

      let first = `000${this.kic_key_index}${this.kic_spec}${this.kic_algorithm}`;
      console.log(`flo-kic - kic_changed - first: ${first}`);
      console.log(`flo-kic - kic_changed - hex: ${this.pad(parseInt(first, 2).toString(16), 2)}`);
