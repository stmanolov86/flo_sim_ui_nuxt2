<template>
  <v-container fluid class="flo-sunburst-card-ota-state ma-0 pa-0">
    <v-row no-gutters>
      <v-col>
...
export default {
  name: "flo-sunburst-card-ota-state",
  data: () => ({
    redrawChart: 0,
    card_ota_state: null,
    stats: [],
    chartSetup: {
      groupBy: ["ota_state", "scp_state"],
      valueBy: "value",
      area: {
  ...
  methods: {
    ...
    onDashboardChange(p_change) {
      ...
      this.card_ota_state = l_cur.statistics.reduce((acc, data) => {
        if (data.stat.startsWith("/cards/ota_state/")) {
          let [start, cards, ota_state, ota_state_val, scp_state, scp_state_val] = data.stat.split("/");
          acc.push({
            ota_state: ota_state_val.toLowerCase(),
            scp_state: scp_state_val.toLowerCase(),
            value: data.value,
          });
        }
        return acc;
    ...
    recalculateMetrics() {
      if (!this.card_ota_state) {
        return;
      }

      this.stats = this.card_ota_state;
    },
  },
};
</script>
