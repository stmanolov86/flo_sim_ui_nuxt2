<template>
  <v-container fluid class="flo-timeline-messages ma-0 pa-0">
    <v-row no-gutters>
      <v-col>
...
export default {
  name: "flo-timeline-messages",
  data: () => ({
    groupby: ["l1", "l2"],
    where: [],
    levels: [
      { level: 1, key: "l1", field: "code", name: "Code" },
      { level: 2, key: "l2", field: "mtype", name: "Type" },
    ],
    x_value_by: "sts",
    y_value_by: "value",
    values: [],
  ...
  methods: {
    ...
    onDashboardChange(p_dashboard) {
      ...
      for (let i = 0; i < l_dashboard_data.length; i++) {
        ...
        let l_messages = l_data.statistics.reduce((acc, data) => {
        ...
        l_dashboard_messages = l_dashboard_messages.concat(l_messages);
      }

      // this.unit = l_unit;
      this.values = l_dashboard_messages;
    },
    get_y_axis_name() {
