<template>
  <v-container fluid class="flo-sunburst-campaign ma-0 pa-0">
    <v-row no-gutters>
      <v-col>
        <v-card flat :outlined="outlined" class="mx-auto ma-0 pa-0">
          ...
          <v-card-text class="ma-0 pa-0">
            <v-card flat>
              <v-row no-gutters>
                <v-col class="pa-1"><floSunburstBasic :chart="chartSetup" :unit="unit" :levels="levels" :rad_factor="4" :unique="unique" :key="`${unique}-${redrawChart}`" :minHeight="minHeight" :maxHeight="maxHeight" /> </v-col>
              </v-row>
            </v-card>
          </v-card-text>
        </v-card>
...
export default {
  name: "flo-sunburst-campaign",
  data: () => ({
    redrawChart: 0,
    campaign_states: null,
    stats: [],
    chartSetup: {
      groupBy: ["state"],
      valueBy: "value",
  ...
  methods: {
    ...
    onDashboardChange(p_change) {
      ...
      this.campaign_states = l_cur.statistics.reduce((acc, data) => {
        if (data.stat.startsWith("/campaigns/state/")) {
          let [start, campaigns, state, state_val] = data.stat.split("/");
    ...
    recalculateMetrics() {
      if (!this.campaign_states) {
        return;
      }

      this.stats = this.campaign_states;
    },
  },
};
</script>
