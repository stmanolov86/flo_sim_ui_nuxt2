...
export default {
  ...
  created() {
    console.log(`flo-plmn-act-calc: created`);
    this.initialize();
  },

  methods: {
    syncParent() {
      console.log(`flo-plmn-act-calc - syncParent - enter`);
      this.$emit("change", this.getActString());
      this.$emit("input", this.getActString());
    },
    getActString() {
      let sb1 = ("00" + this.b1v.toString(16)).substr(-2);
      // console.log(`flo-plmn-act-calc: getActString ${sb1}`);
      let sb2 = ("00" + this.b2v.toString(16)).substr(-2);
      // console.log(`flo-plmn-act-calc: getActString ${sb2}`);
      return `${sb1}${sb2}`;
    },
    setActString(event) {
      console.log(`flo-plmn-act-calc: setActString ${event}`);
      if (event.length != 4) {
        return;
      }
    ...
    b1Changed() {
      // console.log(`flo-plmn-act-calc: b1 - changed - ${this.b1v.toString(2)}`);
      this.version++;
      this.syncParent();
    },
    b2Changed() {
      // console.log(`flo-plmn-act-calc: b2 - changed - ${this.b2v.toString(2)}`);
      this.version++;
      this.syncParent();
    },
    getNgRan() {
    ...
    getEUtran() {
      ...
      // console.log(`flo-plmn-act-calc: getEUtran - eutran - all -${eutran}`);
      // console.log(`flo-plmn-act-calc: getEUtran - eutran - ${eutran & b1_eutran}`);

      if ((eutran & b1_eutran) == 0) {
        // console.log(`flo-plmn-act-calc: getEUtran - 1 - E-UTRAN Not Selected`);
        return 1;
      } else {
        // eutran in use
        if ((eutran & b1_eutran_nb_s1) == 0) {
          ...
          if ((eutran & b1_eutran_wb_s1) == 0) {
            ...
            // console.log(`flo-plmn-act-calc: getEUtran - 2 (all) - E-UTRAN in NB and WB S1`);
            return 2;
          } else {
            // only wb in use
            // console.log(`flo-plmn-act-calc: getEUtran - 4 - E-UTRAN in WB S1 Only`);
            return 4;
          }
        } else {
          if ((eutran & b1_eutran_wb_s1) == 0) {
            // only nb in use
            // console.log(`flo-plmn-act-calc: getEUtran - 3 - E-UTRAN in NB S1 Only`);
            return 3;
          } else {
            // wb in use -- means that they both in use
            // console.log(`flo-plmn-act-calc: getEUtran - 2 (none) - E-UTRAN in NB and WB S1`);
            return 2;
          }
        }
      }
    ...
    getGsm() {
      ...
      // console.log(`flo-plmn-act-calc: getGsm - gsm - all -${gsm}`);
      // console.log(`flo-plmn-act-calc: getGsm - gsm h - ${gsm & b2_gsm_h}`);

      if ((gsm & b2_gsm_h) == 0) {
        // console.log(`flo-plmn-act-calc: getGsm - 1 - GSM and EC-GSM-IoT Not Selected`);
        return 1;
      } else {
        // gsm in use
        if ((gsm & b2_gsm_l) == 0) {
          if ((gsm & b2_ec_gsm_iot) == 0) {
            // console.log(`flo-plmn-act-calc: getGsm - 2 - none - GSM and EC-GSM-IoT`);
            return 2;
          } else {
            // console.log(`flo-plmn-act-calc: getGsm - 4 - EC-GSM-IoT Only`);
            return 4;
          }
        } else {
          if ((gsm & b2_ec_gsm_iot) == 0) {
            // console.log(`flo-plmn-act-calc: getGsm - 3 - GSM without EC-GSM-IoT`);
            return 3;
          } else {
            // console.log(`flo-plmn-act-calc: getGsm - 2 - both - EC-GSM-IoT Only`);
            return 2;
          }
        }
      }
    ...
    initialize() {
      console.log(`flo-plmn-act-calc - initialize - value: ${this.value}`);
      if (this.value) {
        if (this.value.length == 4) {
          this.b1v = parseInt(this.value.substr(0, 2), 16);
          this.b2v = parseInt(this.value.substr(2, 2), 16);
        }
      }
      console.log(`flo-plmn-act-calc - initialize - b1v: ${this.b1v}, b2v: ${this.b2v}`);
    },
  },
};
</script>
